...
import type { EspeakPhonemizer } from './espeak-wasm';
...
export interface USTConverterOptions {
  phonemizer?: EspeakPhonemizer | null;
...
export interface ConversionResult {
  output: string;
  fullOutput: string;
  notationType: 'japanese' | 'xsampa' | 'english' | 'ipa';
  notationCounts: {
    japanese: number;
    xsampa: number;
    english: number;
  ...
  unknownLyrics: string[];
  unknownIPA: string[];
  phonemizerLog: [string, string][];
  espeakAvailable: boolean;
  notesCount: number;
  tempo: number;
  stripLongSilencesEnabled: boolean;
  maxSilenceMs: number;
  strippedSilences: number;
}
...
export class USTConverter {
  ...
  maxSilenceMs: number;
  strippedSilences = 0;
  unknownIPA = new Set<string>();
  unknownLyrics = new Set<string>();
  phonemizerLog: [string, string][] = [];
  notationType: 'japanese' | 'xsampa' | 'english' | 'ipa' | null = null;
  espeakAvailable: boolean;
  notationCounts = {
    japanese: 0,
    xsampa: 0,
  ...
  constructor(noteOffset = 0, options: USTConverterOptions = {}) {
  ...
  detectNotationType(lyrics: string[]): 'japanese' | 'xsampa' | 'english' | 'ipa' {
    ...
    const keys: ('japanese' | 'xsampa' | 'english' | 'ipa')[] = ['japanese', 'xsampa', 'english', 'ipa'];
    return keys.reduce((best, key) => {
      if (this.notationCounts[key] > this.notationCounts[best]) {
        return key;
      }
      return best;
    }, 'japanese' as 'japanese' | 'xsampa' | 'english' | 'ipa');
  }

  xsampaToIpaConvert(xsampaString: string) {
    const result = [];
    let i = 0;
  ...
  ipaToDectalk(ipaString: string): [string, boolean][] {
  ...
  compensateDuration(phoneme: string, requestedDur: number) {
  ...
  midiToPitch(midiNote: number) {
    const shifted = midiNote + this.noteOffset;
    if (Object.prototype.hasOwnProperty.call(MIDI_TO_INDEX, String(shifted))) {
      return MIDI_TO_INDEX[String(shifted)];
    }
    const hz = 440.0 * (2 ** ((shifted - 69) / 12));
    return Math.round(hz);
  }

  cleanEnglishLyric(lyric: string) {
  ...
  lyricToIpa(lyric: string, englishIpaMap: Record<string, string> | null) {
    let cleaned = lyric.trim();
  ...
  formatOutput(notes: USTNote[], englishIpaMap: Record<string, string> | null) {
    const result = [];
  ...
  getResults(output: string, notes: USTNote[]): ConversionResult {
    return {
      output,
      fullOutput: `[:phone on] ${output}`,
      notationType: this.notationType as 'japanese' | 'xsampa' | 'english' | 'ipa',
  ...
  convert(text: string): ConversionResult {
    ...
    return this.getResults(output, notes);
  }

  async buildEnglishIpaMap(notes: USTNote[]) {
  ...
  async convertAsync(text: string): Promise<ConversionResult> {
    const notes = this.parseUST(text);
...
export function decodeUSTBuffer(arrayBuffer: ArrayBuffer) {
...
export function convertUSTText(text: string, noteOffset = 0, options: USTConverterOptions = {}): ConversionResult {
  const converter = new USTConverter(noteOffset, options);
  return converter.convert(text);
}

export async function convertUSTTextAsync(text: string, noteOffset = 0, options: USTConverterOptions = {}): Promise<ConversionResult> {
  const converter = new USTConverter(noteOffset, options);
  return await converter.convertAsync(text);
}
